...
test.describe('Job Application Inbox & Mail Reader', () => {
  test('paginates thread list with cursor, loads full body for selected threads, verifies sanitization, and handles cancellation', async ({
    ...
    const searchInput = page.getByRole('textbox', { name: 'Search mail' });
    ...
    const messageDetail = page.locator('.inbox-thread-detail');
    await expect(messageDetail).toBeVisible();
    await expect(messageDetail).toContainText('Plain text body content for senior software engineer interview.');
    ...
    const richBody = page.locator('.inbox-message__body-rich');
    const hasDangerousTags = await richBody.evaluate((el) =>
      Boolean(el.querySelector('script') || el.querySelector('style')),
    );
    expect(hasDangerousTags, 'DOMPurify HTML sanitizer must strip script and style elements').toBe(false);
    ...
    await page.getByRole('button', { name: /Technical Interview Code Sample and Compensation Table/ }).click();
    ...
    await expect(page.getByText('This message has no body content.')).toBeVisible();

    // 9. Select Provider Error Thread
    await page.getByRole('button', { name: /Provider Error Failure/ }).click();
    await expect(page.getByText('Internal provider failure')).toBeVisible();

    // 10. Select Provider Truncated Thread
    await page.getByRole('button', { name: /Large Diagnostic Export Attachment/ }).click();
    await expect(page.getByText('Message truncated by provider (size_limit).')).toBeVisible();
    ...
    await expect(page.getByText('ada@local.test')).toBeVisible();
