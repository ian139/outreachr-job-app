...
**Disconnect** deletes the encrypted local token but does not revoke the provider-side consent grant. To revoke access as well, remove the app under [Google Account connections](https://myaccount.google.com/connections) or, for a Microsoft work/school account, [Microsoft My Apps](https://myapps.microsoft.com/). An organization administrator may need to revoke admin-consented Microsoft permissions.
