...
7. Hosted macOS releases strictly require complete Developer ID signing and notarization secret groups in `production-release` for both hosted macOS architectures (`macos-x64` and `macos-arm64`). Windows Authenticode secrets remain optional; an absent Windows group produces conspicuously labeled unsigned artifacts. Partial credential groups fail.
...
9. Watch all six target-native jobs. Verify that `Outreachr_Investor_Seed.sqlite` is absent from generated and packaged resources. Do not approve publication when any package, trust disclosure, resource, seed-absence assertion, checksum, provenance, attestation, or smoke check fails.
...
11. Download the public assets on each operating system, inspect `SIGNING-STATUS-<target>.json`, verify the corresponding SHA-256 manifest and GitHub attestation, install, complete a clean-vault launch, and record the result. Expect SmartScreen warnings only for explicitly unsigned Windows files; hosted macOS artifacts are mandatory signed and notarized.

Release automation cannot create vendor certificates, a GitHub remote, repository protections, or a hosted release without the maintainer’s external accounts. Mandatory macOS Developer ID signing and notarization and optional Windows Authenticode certificates upgrade distribution trust, but application binaries do not require cloud services to run.
